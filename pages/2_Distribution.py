...
# --- WORKFLOW 1: TIER 1 ADMIN VERIFICATION PIPELINE ---
if current_role == 'Admin':
    st.subheader("📥 Tier 1: Pending Inbound Receipts (Requiring Admin Verification)")
    ...
    if not pending_df.empty:
        for idx, row in pending_df.iterrows():
            ...
            if c1.button("Forward to Col GSEM", key=f"fwd_rc_{row['receipt_id']}_{idx}", use_container_width=True, type="primary"):
        ...
        st.info("No incoming field unit receipt items require your initial confirmation.")
    st.divider()

# --- WORKFLOW 1.5: TIER 2 COMMANDER FINAL SANCTION PIPELINE ---
elif current_role == 'Commander':
    st.subheader("🛑 Tier 2: Final Command Sanction (Awaiting Col GSEM)")
    
    query_gsem = """
        SELECT r.receipt_id, r.model_name, r.serial_number, o.node_name, r.arrival_date, r.letter_number 
        FROM Receipt_Requests r
        JOIN Org_Structure o ON r.unit_id = o.node_id
        WHERE r.status = 'Pending_GSEM'
    """
    gsem_df = pd.read_sql_query(query_gsem, conn)
    
    if not gsem_df.empty:
        for idx, row in gsem_df.iterrows():
            ...
            c1, c2, _ = st.columns([2, 2, 6])
            if c1.button("Grant Final Sanction", key=f"app_rc_{row['receipt_id']}_{idx}", use_container_width=True, type="primary"):
                # Only upon final sanction does the asset officially enter the distributed inventory
                cursor.execute("""
                ...
                conn.commit()
                st.rerun()
            if c2.button("Return to Admin", key=f"ret_rc_{row['receipt_id']}_{idx}", use_container_width=True):
                cursor.execute("UPDATE Receipt_Requests SET status='Pending Verification' WHERE receipt_id=?", (row['receipt_id'],))
                conn.commit()
                st.rerun()
            st.markdown("</div>", unsafe_allow_html=True)
    else:
        st.info("No vetted indents are pending your final authorization.")
    st.divider()
...
if auth_nodes:
    ...
    if not unit_list.empty:
        selected_unit_id = st.selectbox(
            ...
            key="unique_formation_selectbox"  # <-- Unique key added here to prevent duplicate ID crashes
        )
...
# --- WORKFLOW 3: FIELD RECONNAISSANCE FORM (HIDDEN FROM ADMIN & COMMANDER) ---
